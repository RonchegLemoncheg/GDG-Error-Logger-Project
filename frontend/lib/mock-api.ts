...
const basicErrors = [
  {
    ...
    code_snippet: {
      before: ["var name = user.Name;", "Console.WriteLine(name.Length);"],
      after: ["if (user?.Name == null) return;", "Console.WriteLine(user.Name.Length);"],
    },
    time: {
    ...
    analysis: {
      ...
      weighted_score: 0.82,
      dependency: ["UserRepository.GetById", "UserValidator.Validate"],
    },
  },
  {
    ...
    code_snippet: {
      before: ["var total = order.Items.Sum(i => i.Price);", "Console.WriteLine(total);"],
      after: ["if(order.Items == null) return;", "var total = order.Items.Sum(i => i.Price);"],
    },
    time: {
  ...
  },
  {
    ...
    code_snippet: {
      before: ["task.Complete();", "Console.WriteLine(task.Status);"],
      after: ["// Corrected call to CompleteTask method", "task.CompleteTask();", "Console.WriteLine(task.Status);"],
    },
    time: {
  ...
  },
  {
    ...
    code_snippet: {
      before: ["var lastPayment = payments[payments.Count - 1];", "ProcessPayment(lastPayment);"],
      after: [
...
export const getErrorById = (id) => {
  return new Promise((resolve) => {
    setTimeout(() =>
      resolve(
        basicErrors.find((error) => error.bug_id === id),
        300,
      ),
    )
  })
